...
$sth=$dbh->prepare("select UsrName,sum(UsrTime),
                    sum(SysTime),
...
sub getstat {
  ...
  $id = shift(@_); 
  $sth = $dbh->prepare("select sum(UsrTime),
                               sum(SysTime),
                               sum(MemUse),count(Cmd),
  ...
  while
(($usrtime,$systime,$memuse,$numcmd,$startend,$usrname)=$sth->fetchrow()
){
...
#       $usrname=~s/.mit.edu//g;
        ...
        printf (TMPF "%-15s %10.2f %10.2f %13.2f %10.1f %10d %6d %10.1f
%10.1f\n",
 
$usrname,$usrtime,$systime,$tottime,$memuse,$numcmd,$Util,$startend,$wai
ttime);
        $tusr=$tusr+$usrtime;
        $tsys=$tsys+$systime;
        $ttot=$ttot+$tottime;
  ...
  printf ("\n%-15s %10.2f %10.2f %13.2f %10.1f %10d %6s %10.1f
%10.1f\n\n","Total:",
